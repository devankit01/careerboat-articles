...
const WP_MAX_CONCURRENT = process.env.NEXT_PHASE === 'phase-production-build' ? 1 : 2;
let wpActive = 0;
const wpWaiters: Array<() => void> = [];

async function withWpSlot<T>(fn: () => Promise<T>): Promise<T> {
  if (wpActive >= WP_MAX_CONCURRENT) {
    await new Promise<void>((resolve) => wpWaiters.push(resolve));
  }
  wpActive += 1;
  try {
    return await fn();
...
async function wpFetch<T>(query: string, variables: Record<string, unknown> = {}): Promise<T> {
  return withWpSlot(async () => {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        ...
        if (isRetryableStatus(response.status)) {
          lastError = new Error(`WP GraphQL request failed: ${response.status}`);
        ...
        const err = error instanceof Error ? error : new Error('WP GraphQL request failed');
        lastError = err;
        if (err.message.includes('failed: 403') || err.message.includes('failed: 429') || err.message.includes('failed: 5')) {
          await sleep(500 * 2 ** attempt);
          continue;
        }
        throw err;
    ...
    throw lastError ?? new Error('WP GraphQL request failed');
  });
}
...
export async function getSitemapPosts(): Promise<SitemapPost[]> {
  ...
  // ponytail: WPGraphQL caps `first` at 100; cursor-walk until empty. Ceiling: 50 pages (5k posts).
  for (let page = 0; page < 50; page += 1) {
    try {
      const data: SitemapQuery = await wpFetch<SitemapQuery>(
        `
        query GetSitemapPosts($first: Int!, $after: String) {
      ...
      const nodes: SitemapPost[] = data.posts?.nodes ?? [];
      posts.push(...nodes.filter((post) => Boolean(post.slug)));
      ...
      console.error('Failed to load sitemap posts from WP GraphQL:', error);
      break;
    }
  }
...
async function firstWorkingQuery(attempts: QueryAttempt[], slug: string, cached: { current: QueryAttempt | null }) {
  const ordered = cached.current
    ? [cached.current, ...attempts.filter((attempt) => attempt.query !== cached.current?.query)]
    : attempts;

  for (const attempt of ordered) {
    try {
      const data = await wpFetch<unknown>(attempt.query, { slug });
      cached.current = attempt;
      return attempt.pick(data);
    } catch {
      if (cached.current?.query === attempt.query) cached.current = null;
    }
  }
...
const faqQueryCache: { current: QueryAttempt | null } = { current: null };
...
export async function getPostFaqsBySlug(slug: string): Promise<PostFaq[]> {
  ...
  const rows = await firstWorkingQuery(attempts, slug, faqQueryCache);
...
const tldrQueryCache: { current: QueryAttempt | null } = { current: null };
...
export async function getPostTldrBySlug(slug: string): Promise<PostTldr | null> {
  ...
  const content = normalizeTldrContent(await firstWorkingQuery(attempts, slug, tldrQueryCache));
...
export function imageAlt(altText?: string | null, fallback = '') {
  const fromWp = decodeHtmlEntities((altText || '').trim());
  return fromWp || decodeHtmlEntities(fallback) || 'Careerboat article image';
}

export function rewriteWpPermalinks(html: string) {
  return html
    .replace(
...
export function publicMediaPath(url?: string | null) {
